...
public class PRNGStreamCipher implements SymmetricCipher {
     ...
     * The seed of the SecureRandom instance is the byte sequence in the {@code key} parameter
     *
     * @param input the byte sequence to be encrypted
     * @param key   the key
     ...
     */
    @Override
    public byte[] encrypt(byte[] input, byte[] key) throws NoSuchAlgorithmException {
    ...
    /**
     * Decrypts the given input byte sequence by calling internally the {@link #encrypt(byte[], byte[])} method. This
     * is a
     * valid
     ...
     */
    @Override
    public byte[] decrypt(byte[] input, byte[] key) throws NoSuchAlgorithmException {
    ...
    public static void main(String[] argv) throws NoSuchAlgorithmException {
        ...
        PRNGStreamCipher prngSC = new PRNGStreamCipher();
        ...
        byte[] encrypted = prngSC.encrypt(text.getBytes(), key.getBytes());
        System.out.println("Encrypted (hex):");
        System.out.println(HexStrings.toHexString(encrypted));

        byte[] decrypted = prngSC.decrypt(encrypted, key.getBytes());
